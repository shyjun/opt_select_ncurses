...
#define MAX_INPUT_LENGTH 100  // Maximum length for numeric input (e.g., "12" or "123")
...
{
    fprintf(stderr, "Usage: %s [in_file=<input_file>] [out_file=<output_file>] [multi_select=yes] [default=<value>] [from_pipe=yes/no] [udp_dbg_port=<udp_dbg_server_port>] [-h  for help]\n", app_name);
}
...
void regex_find()
{
    ...
    for (i = 0; i < num_options; ++i) {
        ...
        if (ret == 0) {
            ...
            last_match_idx = i;
            udp_dbg("%d:num_options=%d The text matches the pattern.highlight=%d,prev_highlight=%d\n",i,num_options,highlight,prev_highlight);
            if(ctrl_n == 1) {
                if(i <= highlight-1) { // item before current highlighted item
                    continue;
                }
            }
            if(ctrl_p == 1) {
    ...
    if((ctrl_p == 1) && (first_match_idx != -1)) {
        udp_dbg("ctrl+p end: i=%d:num_options=%d .highlight=%d,prev_highlight=%d,found_highlight=%d\n",
